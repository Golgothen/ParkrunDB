...
gmaps = googlemaps.Client(key='AIzaSyAWsQDjsyReEypMGbt-54LNwwHmxEzSSys')


c = Connection(sender_config)

def gelolocate_parkruns():
...
def measure_distances(athleteid, reload = False):
    data = c.execute('SELECT AthleteID, GivenAddress, Latitude, Longitude, State, Country FROM Athletes WHERE AthleteID = {}'.format(athleteid))
    for row in data:
        if row['State'] is None:
            ...
            if len(geocode_result)>0:
                ...
                orig = geocode_result[0]['geometry']['location']
        else:
            state = row['State']
            country = row['Country']
        ...
        if not reload:
            ...
            parkruns = [x for x in parkruns if x['ParkrunID'] not in parkrunDistances]
        ...
        while len(parkruns)>0:
            ...
            days_ahead = 5 - arrivalTime.weekday()
            if days_ahead <= 0:
                days_ahead += 7
            arrivalTime = (arrivalTime + datetime.timedelta(days_ahead)).replace(hour = 8, minute = 0)
            d = gmaps.distance_matrix(origins = orig, destinations = dests['location'], mode = "driving", avoid = "ferries", arrival_time = arrivalTime, units = "metric")
...
if __name__ == '__main__':
    ...
    measure_distances(2982399, True)
